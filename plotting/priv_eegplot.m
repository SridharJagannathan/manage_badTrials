...
function [outvar1] = priv_eegplot(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
%        % Check  consistency of freqs
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   try, g.badchan;          catch, g.badchan	= []; end;
   ...
   for index=1:length(gfields)
      ...
      case {'spacing', 'srate' 'eloc_file' 'winlength' 'position' 'title' 'plottitle' ...
               'trialstag'  'winrej' 'command' 'tag' 'xgrid' 'ygrid' 'color' 'badchan' 'colmodif'...
   ...
   if ~iscell(g.color)
       ...
       g.badchan = find(strcmp(g.color,'r'));
   end;
...
u(22) = uicontrol('Parent',figh, ...
  ...
  m(11) = uimenu('Parent',m(1),'Label','Data select/mark', 'tag', 'displaywin', ...
	...
	% set bad channels - Modified by Shri
	uimenu('Parent',m(11),'Label','Mark channels', 'enable', 'on', ...
    'checked', 'off', 'Callback', ...
  	[' priv_eegplot(''badelect'');' ...
    'g = get(gcbf, ''userdata'');' ...
  	'g.setelectrode = ~g.setelectrode;' ...
  	'set(gcbf, ''userdata'', g); ' ...
    'if ~g.setelectrode set(gcbo, ''checked'', ''on'');' ...
    'else set(gcbo, ''checked'', ''off''); end;'...
    ' clear g;'] )

    % set bad channels - Modified by Shri
% 	uimenu('Parent',m(11),'Label','Recon Electrodes/channels', 'enable', 'on', ...
%     'checked', 'off', 'Callback', ...
%   	[' priv_eegplot(''reconelect'');' ...
%     'g = get(gcbf, ''userdata'');' ...
%   	'g.setelectrode = ~g.setelectrode;' ...
%   	'set(gcbf, ''userdata'', g); ' ...
%     'if ~g.setelectrode set(gcbo, ''checked'', ''on'');' ...
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
  ...
  case 'badelect' % load channels %Modified by Shri..
     
      fig = gcf;
      g = get(gcf,'UserData');
      result = inputdlg2( ...
      { 'ReSet Channel# :' } , 'Set the bad or good channels', 1,  {NaN} );
   if ~isnan(str2double(result)) 
       channelidx = str2double(result);
       if(strcmp(g.color(channelidx),'k'))
           g.color(channelidx) = {'r'};
           fprintf('--> Channel %d is now bad..\n',channelidx);
       else
           g.color(channelidx) = {'k'};
           fprintf('--> Channel %d is now good..\n',channelidx);
       end
       g.badchan = find(strcmp(g.color,'r'));
       set(gcf, 'UserData', g);
       eegplot('drawp',0);
...
%    case 'reconelect' % load channels %Modified by Shri..
%      
%       fig = gcf;
%       g = get(gcf,'UserData');
%       prmpt = { 'ReSet Trial# :','ReSet Electrode# :' };
%       dlgtitle = 'Recon Trials/Electrodes';
%       result = inputdlg2(prmpt , dlgtitle, 1, {NaN,NaN});
%    if (~isnan(str2double(result{1})) && ~isnan(str2double(result{2})))
%        trialidx = str2double(result{1});
%        channelidx = str2double(result{2});
%        if(strcmp(g.color(channelidx),'k'))
%            g.color(channelidx) = {'r'};
%            fprintf('--> Channel %d is now bad..\n',channelidx);
%        else
